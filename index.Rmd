...
# Response variable has to be a factor, and it already is:
str(training$classe)
```
...
dim(little_train)
little_test <- little_test[ ,c(-throw_away(little_test))]
dim(little_train); dim(little_test)
```
---
...
plot(fit.pruned)
text(fit.pruned, cex=0.5, use.n = T, xpd = T)
prp(fit.pruned,varlen = 0, clip.right.labs = T, cex=0.5, extra = 1, type = 0, nn = T, yesno = T, ni = 1, fallen.leaves = T,xflip = T, digits = 0, faclen = 0)
...
```{r}
table(little_test$classe, predicted = p)
```