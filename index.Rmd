...
```{r}
empty <- rep(0, length(little_train))
for(i in 1:length(little_train)){
    if (sum((is.na(little_train[ ,i])) / nrow(little_train)) > 0.01){
        empty[i] <- i  
    } 
    else{
        empty[i] <- NA
...
```{r}
fit <- rpart(classe ~ ., data=train, method="class") # Classification tree
plot(fit)
text(fit,cex=0.6)
```